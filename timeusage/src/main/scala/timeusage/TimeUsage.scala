...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val tail = columnNames.tail.map(StructField(_, DoubleType, false))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val needs = List("t01", "t03", "t11", "t1801", "t1803")
    val working = List("t05", "t1805")
    val other = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val otherExclude = List("t1801", "t1803", "t1805")

    val nc = columnNames.filter(c => needs.count(n => c.startsWith(n)) > 0).map(col)
    val wc = columnNames.filter(c => working.count(n => c.startsWith(n)) > 0).map(col)
    val oc = columnNames
      .filter(c => other.count(n => c.startsWith(n)) > 0)
      .filterNot(c => otherExclude.count(n => c.startsWith(n)) > 0).map(col)
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map { timeUsage =>
      TimeUsageRow(
        timeUsage.getAs[String]("working"),
        timeUsage.getAs[String]("sex"),
        timeUsage.getAs[String]("age"),
        timeUsage.getAs[Double]("primaryNeeds"),
        timeUsage.getAs[Double]("work"),
        timeUsage.getAs[Double]("other")
      )
    }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map { case ((working, sex, age), primaryNeeds, work, other) =>
          TimeUsageRow(working, sex, age, roundUp(primaryNeeds), roundUp(work), roundUp(other))
      }
  }
}
