...
object WikipediaRanking {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map(lang => lang -> occurrencesOfLang(lang, rdd)).sortWith { case ((_, a), (_, b)) => b < a }

  /* Compute an inverted index of the set of articles, mapping each language
   * to the Wikipedia pages in which it occurs.
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article => langs.filter(article.text.split(" ").contains(_)).map(_ -> article)).groupByKey
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(article => langs.map(lang => lang -> (if(article.text.split(" ").contains(lang)) 1 else 0)))
    .reduceByKey((a, b) => a + b)
    .sortBy(_._2, false)
    .collect().toList
  }
